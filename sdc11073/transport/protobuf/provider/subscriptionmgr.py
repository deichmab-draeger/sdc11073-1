...
class _DevSubscription(object):
    ...
    def sendNotificationReport(self, report):
...
#    NotificationPrefixes = [Prefix.S12, Prefix.PM, Prefix.WSA, Prefix.WSE]
    ...
    def __init__(self, sdc_definitions, max_subscription_duration=None, log_prefix=None):
        ...
        self._logger = loghelper.getLoggerAdapter('sdc.grpc.dev.subscrMgr', self.log_prefix)
    ...
    def onSubscribeRequest(self, action_strings):
        s = _DevSubscription(self._max_subscription_duration, action_strings)
    ...
    def sendEpisodicMetricReport(self, updatedMetricStates, nsmapper, mdibVersion, sequenceId):
        ...
        self._set_mdib_version_group(oneof_report.abstract_metric_report.abstract_report.a_mdib_version_group, sequenceId, mdibVersion)
        p_report_part = oneof_report.abstract_metric_report.report_part.add()
        for sc in updatedMetricStates:
            p_st = p_report_part.metric_state.add()
            generic_state_to_p(sc, p_st)
        for s in subscribers:
            s.sendNotificationReport(report)
    ...
    def sendEpisodicComponentStateReport(self, updatedComponentStates, nsmapper, mdibVersion, sequenceId):
        ...
        self._set_mdib_version_group(oneof_report.abstract_component_report.abstract_report.a_mdib_version_group, sequenceId, mdibVersion)
        for sc in updatedComponentStates:
    ...
    def sendEpisodicContextReport(self, updatedContextStates, nsmapper, mdibVersion, sequenceId):
        ...
        self._set_mdib_version_group(oneof_report.abstract_context_report.abstract_report.a_mdib_version_group, sequenceId, mdibVersion)
        for sc in updatedContextStates:
    ...
    @staticmethod
    def _set_mdib_version_group(mdib_version_group, sequence_id, mdib_version , instance_id=None):
        mdib_version_group.a_sequence_id = sequence_id
        if mdib_version is not None:
            mdib_version_group.a_mdib_version.value = mdib_version
        if instance_id is not None:
            mdib_version_group.a_instance_id.value = instance_id
    ...
    def _mkDescriptorUpdatesReportPart(self, report, modification_type, descriptors, updated_states):
        ...
        for descrContainer in descriptors:
            ...
            p_descr = p_report_part.descriptorx_xx.add()
            generic_descriptor_to_p(descrContainer, p_descr)
            ...
            for stateContainer in relatedStateContainers:
                ...
                generic_state_to_p(stateContainer, p_state)
    ...
    def _sendNotificationReport(self, subscription, report, action):
        subscription.sendNotificationReport(report)
